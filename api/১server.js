...
const SESSION_COOKIE = "smm_admin_session";
...
async function getFirebaseCustomToken(uid = "smm-boost-pro-admin", email = "") {
  ...
  return signJwt(
    { alg: "RS256", typ: "JWT" },
    {
      ...
      exp: now + 3600,
      uid,
      claims: { admin: true, role: "admin", email },
    },
    account.private_key,
  );
}
  ...
  if (!response.ok) {
    ...
    const error = new Error(
      code === "EMAIL_NOT_FOUND" || code === "INVALID_PASSWORD"
        ...
        : code || "Firebase Authentication failed",
    );
    error.code = code;
    throw error;
  }
...
function readLegacyCatalog() {
  ...
  return {
    categories: readJsonConstant("HARDCODED_CATEGORIES") || [],
    services: readJsonConstant("HARDCODED_SERVICES") || [],
    settings: readJsonConstant("HARDCODED_SETTINGS") || {},
  };
}
...
function orderMatchesIdentifier(order, query, contains = false) {
  ...
  return [ ...orderIdentifiers(order), normalizeOrderIdentifier(order?.trxId), normalizeOrderIdentifier(order?.customerPhone) ]
    .some((value) => value === normalizedQuery || value.replace(/^ex-/, "") === normalizedQuery.replace(/^ex-/, "") || (contains && value.includes(normalizedQuery)));
}

async function getCatalog() {
  const legacy = readLegacyCatalog();
  try {
    const [categories, services, settings, announcements] = await Promise.all([
      firebaseRequest("categories"),
      firebaseRequest("services"),
      firebaseRequest("siteSettings"),
      firebaseRequest("announcements"),
    ]);
    return {
      categories: categories && Object.keys(categories).length ? collectionAsArray(categories) : legacy.categories,
      services: services && Object.keys(services).length ? collectionAsArray(services) : legacy.services,
      settings: { ...legacy.settings, ...(settings || {}) },
      announcements: announcements && Object.keys(announcements).length ? collectionAsArray(announcements) : [],
    };
  } catch {
    return legacy;
  }
...
async function seedLegacyCatalog() {
  ...
  try {
    ...
    if (!categories || Object.keys(categories).length === 0) {
      const categoryMap = Object.fromEntries(legacy.categories.map((item) => [item.id, { ...item, updatedAt: Date.now() }]));
      await firebaseRequest("categories", { method: "PUT", body: JSON.stringify(categoryMap) });
    }
    if (!services || Object.keys(services).length === 0) {
      const serviceMap = Object.fromEntries(legacy.services.map((item) => [item.id, { ...item, updatedAt: Date.now() }]));
      await firebaseRequest("services", { method: "PUT", body: JSON.stringify(serviceMap) });
    }
    const settings = await firebaseRequest("siteSettings");
    if (!settings || Object.keys(settings).length <= 1 || settings.siteName === "SMM BOOST PRO Marketplace") {
      await firebaseRequest("siteSettings", { method: "PUT", body: JSON.stringify({ ...legacy.settings, ...(settings || {}), siteName: "Social Service Panel", updatedAt: Date.now() }) });
    }
  } catch (error) {
    console.error(`Firebase seed skipped: ${error.message}`);
  }
}
...
function serveStatic(req, res, urlPath) {
  const filePath = safeAssetPath(urlPath);
  ...
  try {
    ...
    json(res, 500, { error: "Could not read asset" });
  }
}
...
async function handleApi(req, res, url) {
  ...
  if (url.pathname === "/api/server/orders" && req.method === "POST") {
    ...
    if (!order || !order.id || !order.serviceName || !order.targetLink) {
      return json(res, 400, { error: "Order is missing required fields" });
    }
  ...
  if (adminDataMatch && ["GET", "PATCH", "PUT", "DELETE"].includes(req.method)) {
    ...
    if (!dataPath.length || !ADMIN_DATA_ROOTS.has(dataPath[0]) || dataPath.some((part) => part === "." || part === "..")) {
      return json(res, 400, { error: "Invalid admin data path" });
    }
  ...
  if (req.method === "POST" && url.pathname === "/api/auth/login") {
    ...
    try {
      ...
      sessions.set(token, {
        ...
        email: firebaseUser.email || body.email.trim(),
  ...
  if (req.method === "GET" && url.pathname === "/api/firebase-token") {
    ...
    if (!session) return;
    try {
      return json(res, 200, { token: await getFirebaseCustomToken(session.uid, session.email) });
    } catch (error) {
  ...
  if (req.method === "POST" && url.pathname === "/api/auth/password") {
    ...
    try {
      ...
      return json(res, 401, { error: error.message || "Could not update password" });
    }
  }
  ...
  if (req.method === "GET" && url.pathname === "/api/firebase-health") {
    if (!requireSession(req, res)) return;
    try {
      const data = await firebaseRequest("siteSettings");
      return json(res, 200, { ok: true, connected: true, hasSettings: Boolean(data) });
    } catch (error) {
...
server.listen(PORT, "0.0.0.0", () => {
  console.log(`Social Service Panel admin server listening on port ${PORT}`);
  seedLegacyCatalog();
});
